...
const RestaurantAdminMenuItemEdit = () => {
    ...
    const fetchMenuItem = async () => {
      try {
        ...
        if (!restaurantId || !menuItemId) {
          console.error('Missing required params:', { restaurantId, menuItemId });
          setError('Missing required parameters for fetching menu item.');
          setIsLoading(false);
          return;
        }
        
        const url = `/menu/${restaurantId}/${menuItemId}`;
        console.log('Making API request to:', url);
        
        // Make the API request to get menu item details
        ...
        if (response.data && response.data.success) {
          ...
          // Set form data with menu item details
          setFormData({
            name: menuItem.name || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(`Using endpoint: /menu/${restaurantId}/${menuItemId}`);
      
      // Update the API endpoint to include restaurantId
      const response = await api.put(`/menu/${restaurantId}/${menuItemId}`, menuItemData);
      
      if (response.data && response.data.success) {
        console.log('Menu item updated:', response.data.data);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      console.log(`Deleting menu item using endpoint: /menu/${restaurantId}/${menuItemId}`);
  ...
  // Show loading spinner while fetching menu item data
  if (isLoading && Object.values(formData).every(field => !field || field === 'Other' || field === true || field === '')) {
    return (
      <div style={container}>
        <div style={card}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={formGroup}>
            <ImageUpload 
              ...
              setImageUrl={(newUrl) => setFormData({...formData, imageUrl: newUrl})} 
...
export default RestaurantAdminMenuItemEdit; 